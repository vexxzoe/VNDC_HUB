        ...
        <div className="flex flex-col gap-4">
          ...
          {activeDoc && (
            <div className="flex flex-col lg:flex-row gap-6">
              <div className="flex-1 bg-white rounded-2xl border border-surface-200 p-6 flex flex-col gap-6">
                ...
                <div className="flex-1 border-2 border-dashed border-surface-200 rounded-xl bg-surface-50 min-h-[320px] flex flex-col items-center justify-center p-8 text-center">
                  {React.createElement(ICON_MAP[activeDoc.icon] || FileText, { 
                    className: clsx("w-16 h-16 mb-4", 
                      activeDoc.type === 'PDF' ? 'text-red-400' :
                      activeDoc.type === 'Excel' ? 'text-green-500' :
                      activeDoc.type === 'Video' ? 'text-blue-400' :
                      'text-purple-400'
                    ) 
                  })}
                  <h3 className="text-lg font-bold text-surface-900 mb-2">{activeDoc.name}</h3>
                  <p className="text-surface-500 font-medium mb-1">Xem trước không khả dụng trong bản demo</p>
                  <p className="text-xs text-surface-400">Tích hợp PDF/Video viewer ở giai đoạn tiếp theo</p>
                </div>
              </div>

              {/* Sidebar */}
              <div className="w-full lg:w-72 shrink-0 bg-primary-50/30 rounded-2xl p-4 flex flex-col gap-4">
